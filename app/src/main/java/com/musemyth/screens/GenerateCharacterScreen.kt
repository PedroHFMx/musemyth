...
fun GenerateCharacterScreen(navController: NavController? = null) {
    ...
    Column (
        ...
        Column(
            ...
            Header(title = "Personagem Gerado:", navController = navController, bgColor = primary)
            ...
            LazyColumn(
                ...
                content = {
                    itemsIndexed(characterTables) { _, item ->
                        val keysList = item.table!!.keys.first();
                        val valuesList: List<*> = randomValuesMap.getValue(keysList)

                        char = keysList to valuesList.first()!!
                        ...
                        Row(
                            ...
                            Box(
                                Modifier
                                    .fillMaxHeight()
                                    .width(57.dp)
                                    .background(primary, ShapeDefaults.ExtraLarge),
                                Alignment.Center
                            ) {
                                Text(
                                    keysList[0] +
